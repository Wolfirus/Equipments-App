...
const ReservationSchema = new mongoose.Schema(
  {
    ...
    ratings: {
      ...
      user_feedback: { type: String, trim: true, maxlength: 500 },
      equipment_feedback: { type: String, trim: true, maxlength: 500 },
    },
    notifications: {
      reminder_sent: { type: Boolean, default: false },
      overdue_notices_sent: { type: Number, default: 0 },
      pickup_reminder_sent: { type: Boolean, default: false },
      return_reminder_sent: { type: Boolean, default: false },
    },
  },
  {
    timestamps: true,
...
/**
 * ✅ FIXED PRE-SAVE
 * We DO NOT check equipment.available_quantity here (it is global and causes false negatives).
 * We check ONLY date conflicts vs total_quantity when reservation is approved/active.
 */
ReservationSchema.pre("save", async function (next) {
  try {
    // Skip validation for cancelled
    if (this.status === "cancelled") return next();

    // Only run on create or when dates/qty/status change
    if (
      !this.isNew &&
    ...
    if (!equipment) return next(new Error("Equipment not found"));

    // Only enforce stock rule for approved/active reservations
    if (!["approved", "active"].includes(this.status)) {
      return next();
    }

    // Find overlapping approved/active reservations for same equipment
    const overlapping = await this.constructor.find({
      _id: { $ne: this._id },
      equipment_id: this.equipment_id,
      status: { $in: ["approved", "active"] },
      $or: [
        { start_date: { $lte: this.start_date }, end_date: { $gte: this.start_date } },
        { start_date: { $lte: this.end_date }, end_date: { $gte: this.end_date } },
        { start_date: { $gte: this.start_date }, end_date: { $lte: this.end_date } },
      ],
    }).select("quantity");

    const reserved = overlapping.reduce((sum, r) => sum + (r.quantity || 0), 0);
    const requested = this.quantity || 1;

    // Compare against total_quantity (real stock)
    if (reserved + requested > equipment.total_quantity) {
...
/**
 * ✅ OPTIONAL: keep available_quantity meaningful (NOW stock)
 * Here we compute active reservations TODAY only.
 */
ReservationSchema.post("save", async function (doc) {
  try {
    ...
    // "active right now" reservations (today in range)
    const activeNow = await mongoose.model("Reservation").find({
    ...
    const usedNow = activeNow.reduce((sum, r) => sum + (r.quantity || 0), 0);

    equipment.available_quantity = Math.max(0, equipment.total_quantity - usedNow);

    // Optional stats fields if they exist in your Equipment model
    if (equipment.usage_stats) {
      equipment.usage_stats.active_rentals = activeNow.length;
      if (doc.status === "completed") {
        equipment.usage_stats.total_rentals = (equipment.usage_stats.total_rentals || 0) + 1;
    ...
    // don't crash server on post hook
    console.error("Reservation post-save error:", err.message);
  }
});

// Static helpers
ReservationSchema.statics.findOverdueReservations = function () {
  return this.find({ status: "active", end_date: { $lt: new Date() } });
};
