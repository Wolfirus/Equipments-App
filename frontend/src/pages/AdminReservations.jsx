...
export default function AdminReservations() {
  ...
  const load = async () => {
    ...
    setErr("");
    try {
      const res = await reservationAPI.getReservations({ page: 1, limit: 200 });
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((r) => {
      const eqName = (r.equipment_id?.name || r.equipment?.name || r.equipment_name || "").toLowerCase();
      const userName = (r.user_id?.name || r.user?.name || r.requested_by || "").toLowerCase();
  ...
  const act = async (id, action) => {
    try {
      ...
      if (action === "reject") await reservationAPI.reject(id);
      if (action === "cancel") await reservationAPI.cancel(id);
      await load();
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">Valider, refuser ou annuler les réservations.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((r) => {
                  const eqName = r.equipment_id?.name || r.equipment?.name || r.equipment_name || "—";
                  const uName = r.user_id?.name || r.user?.name || r.requested_by || "—";
